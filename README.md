...
<!-- Preconnect to the APIs we'll hit on every scan — saves the TLS handshake on the first request -->
...
<!-- Firebase Realtime Database (compat build — exposes `firebase` global) -->
...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .room-panel {
    margin-top: 32px;
    padding: 22px 24px;
    background: linear-gradient(180deg, rgba(212,168,58,0.06) 0%, rgba(212,168,58,0.02) 100%);
    border: 1px solid rgba(212,168,58,0.35);
    width: 100%;
    max-width: 520px;
    display: flex;
  ...
  .room-btn:hover { background: rgba(212,168,58,0.12); }
  .room-current {
    display: flex;
    align-items: center;
    ...
    border: 1px dashed rgba(212,168,58,0.4);
  }
  .room-current-code {
    font-family: 'IBM Plex Mono', monospace;
    font-size: 18px;
  ...
  .room-chip .rc-code { color: var(--paper); font-weight: 600; letter-spacing: 0.24em; }
  .hud-top .room-chip {
    border-color: var(--hud-rule);
    color: var(--hud-cyan);
    background: transparent;
  }
  .hud-top .room-chip .rc-code { color: var(--hud-cream); }
...
<div id="picker">
  ...
  <div class="picker-cards">
    <button class="pcard" id="pickPhone" onclick="goto('phone')">
    ...
    <button class="pcard" id="pickGlasses" onclick="goto('glasses')">
      <div class="pcard-icon">◇</div>
      <h3>Glasses HUD</h3>
  ...
  <div class="config-banner" id="configBanner" hidden>
    ...
    Running in local-only mode — phone and glasses will only sync if they're in the same browser. To sync across devices, fill in the <code>FIREBASE_CONFIG</code> block at the top of the <code>&lt;script&gt;</code> tag with your project credentials. See README in the picker note below.
  </div>

  <div class="room-panel" id="roomPanel">
    <h4>Pair phone &amp; glasses</h4>
    <p>Enter the same room code on both surfaces, or generate a new one and copy it across. Codes are 4 characters, easy to read aloud. Stored locally on each device.</p>

    <div class="room-row" id="roomEntry">
      <input class="room-input" id="roomInput" maxlength="4" placeholder="ABCD" autocomplete="off" spellcheck="false">
      <button class="room-btn" id="roomJoinBtn" type="button">Join</button>
      <button class="room-btn" id="roomGenBtn" type="button">Generate</button>
    </div>

    <div class="room-current" id="roomCurrent" hidden>
      <div>
        <div class="room-current-lbl">Paired in room</div>
        <div class="room-current-code" id="roomCurrentCode">—</div>
      </div>
      <button class="room-link" type="button" onclick="clearRoom()">Change</button>
    </div>
  </div>

  <p class="picker-note">
    Open the phone surface on your phone and the glasses surface on your MRBD glasses.
    Pair them by entering the same room code on both, then scan. Codes are 4 characters
    and persist locally on each device. Add <code>?demo</code> to either URL to preload
    sample scans. For cross-device sync to work you need Firebase configured — see the
    <code>FIREBASE_CONFIG</code> block at the top of the &lt;script&gt; tag.
  </p>
</div>
...
<div id="phone" hidden>
  <div class="ph-head">
    ...
    <div style="display: flex; flex-direction: column; align-items: flex-end; gap: 4px;">
      ...
      <span class="room-chip" id="phoneRoomChip" hidden>Room · <span class="rc-code" id="phoneRoomCode">—</span></span>
    </div>
  </div>
  ...
  <div class="hud" id="hudRoot">
    <div class="hud-top">
      ...
      <div class="hud-status">
        <span class="room-chip" id="glassesRoomChip" hidden style="margin-right: 6px;"><span class="rc-code" id="glassesRoomCode">—</span></span>
...
/* ═════════════════════════════════════════════════════════════════════
   FIREBASE CONFIG  —  fill these in from your Firebase project's web
   app settings (Project settings → General → Your apps → Web app).
   You ONLY need a Realtime Database, not Firestore.
   ...
   These rules are intentionally open within the rooms node. The 4-char
   room code is your only access control — fine for a personal demo,
   not for anything you'd publish. Tighten with Firebase Auth later.

   Leave the placeholders as-is to run in local-only mode (no cross-
...
if (FIREBASE_READY) {
  try {
    ...
    console.info('Firebase ready · syncing via', FIREBASE_CONFIG.databaseURL);
  } catch (e) {
...
/* ═════════════════════════════════════════════════════════════════════
   ROOM CODE  —  the shared key both surfaces use to talk to each other
   ═════════════════════════════════════════════════════════════════════ */
const ROOM_KEY = 'mtg-room-code';
...
function generateRoomCode() {
  ...
  for (let i = 0; i < 4; i++) out += ROOM_ALPHABET[arr[i] % ROOM_ALPHABET.length];
  return out;
}

function getRoom() {
  return (localStorage.getItem(ROOM_KEY) || '').toUpperCase().trim();
}
...
function setRoom(code) {
  ...
  // Re-render and re-subscribe if a surface is already active
  detachGlassesListener();
  syncRoomChips();
  refreshPickerRoomUi();
  if (currentMode() === 'glasses') initGlasses(true);
  if (currentMode() === 'phone') refreshGlasses(); // no-op on phone, harmless
  return clean;
}
...
function clearRoom() {
  ...
  syncRoomChips();
  refreshPickerRoomUi();
  if (currentMode()) { location.hash = ''; render(); }
}

/* ═════════════════════════════════════════════════════════════════════
   STORAGE  —  Firebase RTDB when configured, else localStorage (single-
   browser only). Both branches expose the same shape: a list of scans.
   ═════════════════════════════════════════════════════════════════════ */
const STORAGE_KEY = 'mtg-scans-v1';
const MAX_RECENT = 8;

function roomScansRef() {
  if (!fbDb) return null;
  const room = getRoom();
  if (!room) return null;
  return fbDb.ref(`rooms/${room}/scans`);
}
...
async function loadScans() {
  ...
  if (fbDb && getRoom()) {
    try {
      const snap = await roomScansRef().limitToLast(MAX_RECENT).once('value');
      return snapToArray(snap);
    } catch (e) {
      console.warn('Firebase loadScans failed, falling back to local:', e);
    }
  }
...
function snapToArray(snap) {
  const val = snap.val();
  if (!val) return [];
  // Object → array, dedupe by oracle_id keeping the most recent, newest first
  const all = Object.entries(val).map(([k, v]) => ({ ...v, _key: k }));
  all.sort((a, b) => (b.scanned_at || 0) - (a.scanned_at || 0));
  const seen = new Set();
  const out = [];
  for (const c of all) {
...
async function pushScan(card) {
  const stamped = { ...card, scanned_at: Date.now() };
  // Firebase path — fire-and-forget so the phone UI updates immediately.
  // The glasses will receive the push via their onValue listener; the phone
  // doesn't actually need to wait for the network round-trip to finish.
  if (fbDb && getRoom()) {
    roomScansRef().push(stamped).catch(e => {
      console.warn('Firebase pushScan failed (continuing on local):', e);
      // If Firebase silently fails, still save locally so it isn't lost.
      try {
        const raw = localStorage.getItem(STORAGE_KEY);
        const existing = raw ? JSON.parse(raw) : [];
        const id = stamped.oracle_id || stamped.id;
        const updated = [stamped, ...existing.filter(c => (c.oracle_id || c.id) !== id)].slice(0, MAX_RECENT);
  ...
  try {
    ...
    console.warn('pushScan local fallback failed', e);
  }
  return stamped;
}
...
function goto(mode) {
  if (!getRoom()) {
    // Auto-generate so the user can just tap a card and go
...
/* ─────────── Picker / room-code UI ─────────── */
function refreshPickerRoomUi() {
  const banner = document.getElementById('configBanner');
  banner.hidden = !!fbDb;

  const room = getRoom();
  const entry = document.getElementById('roomEntry');
  const current = document.getElementById('roomCurrent');
  if (room) {
    entry.hidden = true;
    current.hidden = false;
    document.getElementById('roomCurrentCode').textContent = room;
  } else {
    entry.hidden = false;
    current.hidden = true;
...
function syncRoomChips() {
  const room = getRoom();
  ['phoneRoomChip', 'glassesRoomChip'].forEach(id => {
    const el = document.getElementById(id);
    if (!el) return;
    el.hidden = !room;
  });
  if (room) {
    const a = document.getElementById('phoneRoomCode'); if (a) a.textContent = room;
    const b = document.getElementById('glassesRoomCode'); if (b) b.textContent = room;
  }
}

function bindPickerOnce() {
  if (bindPickerOnce._done) return;
  bindPickerOnce._done = true;
  document.getElementById('roomGenBtn').addEventListener('click', () => {
    setRoom(generateRoomCode());
  });
  document.getElementById('roomJoinBtn').addEventListener('click', () => {
    const v = document.getElementById('roomInput').value;
    if (!setRoom(v)) {
      alert('Room codes are 4 characters (letters + digits).');
    }
  });
  document.getElementById('roomInput').addEventListener('keydown', (e) => {
    if (e.key === 'Enter') document.getElementById('roomJoinBtn').click();
  });
...
/* ═════════════════════════════════════════════════════════════════════
   ...
   The strategy is two-step for resilience against OCR noise:
   1. Try /cards/named?fuzzy=<text> — fast, single hop, works for any
      input that's "close enough" by Scryfall's similarity threshold.
   2. If that 404s (it's stricter than people expect), fall back to
      /cards/autocomplete?q=<text> to get the top candidate name, then
      look THAT name up exactly. This recovers from things like
      "RAGAYAN NIMBLE PIIFERER" → "Ragavan, Nimble Pilferer".
   ═════════════════════════════════════════════════════════════════════ */
async function scryfallLookup(query) {
  // Step 1: fuzzy
  const fuzzyUrl = `https://api.scryfall.com/cards/named?fuzzy=${encodeURIComponent(query)}`;
  let res = await fetch(fuzzyUrl, { headers: { 'Accept': 'application/json' } });
  if (res.ok) return normaliseCard(await res.json());
  if (res.status !== 404) throw new Error(`Scryfall returned ${res.status}`);

  // Step 2: autocomplete → top candidate → exact lookup
  const acUrl = `https://api.scryfall.com/cards/autocomplete?q=${encodeURIComponent(query)}&include_extras=false`;
  const acRes = await fetch(acUrl, { headers: { 'Accept': 'application/json' } });
  if (!acRes.ok) throw new Error(`No card matches "${query}"`);
  const ac = await acRes.json();
  const candidate = ac && ac.data && ac.data[0];
  if (!candidate) throw new Error(`No card matches "${query}"`);

  const exactUrl = `https://api.scryfall.com/cards/named?exact=${encodeURIComponent(candidate)}`;
  res = await fetch(exactUrl, { headers: { 'Accept': 'application/json' } });
  if (!res.ok) throw new Error(`Found candidate "${candidate}" but lookup failed`);
  return normaliseCard(await res.json());
}
...
let ocrWorker = null;
// Native Shape Detection API — fast and free on Android Chrome. Falls back to Tesseract.
const NATIVE_TEXT_DETECTOR = ('TextDetector' in window);
...
async function initPhone() {
  ...
  // Warm up Tesseract in the background so the first scan doesn't pay the
  // ~2s engine-load cost. Native TextDetector doesn't need warming up.
  if (!NATIVE_TEXT_DETECTOR) {
    setTimeout(() => { ensureTesseract().catch(() => {}); }, 600);
  }
}
...
async function startCamera() {
  ...
  try {
    ...
    // Ask for the biggest reasonable frame the device can give us. Higher
    // resolution = sharper title text = better OCR.
    ...
    // ImageCapture gives us full-sensor stills via takePhoto() — typically
    // 4x the pixel count of the preview stream. Not on Safari/iOS though.
    if ('ImageCapture' in window) {
      try { imageCapture = new ImageCapture(videoTrack); } catch (_) { imageCapture = null; }
    }
    // Try to enable continuous autofocus if the device exposes it.
    try {
    ...
    const engine = NATIVE_TEXT_DETECTOR ? 'native TextDetector' : 'Tesseract.js';
    setStatus(`Line up the card so the title fills the dashed band. OCR: ${engine}.`, 'ok');
  } catch (e) {
...
/**
 * Grab a frame at the highest resolution we can. Returns a {bitmap,width,height}
 * source we can crop from. Prefers ImageCapture.takePhoto() (full sensor),
 * falls back to grabFrame() (preview-res ImageBitmap), then to a <canvas>
 * drawImage of the video element (always works, lowest res).
 */
async function grabBestFrame(video) {
  if (imageCapture && imageCapture.takePhoto) {
    try {
      const blob = await imageCapture.takePhoto();
      const bmp = await createImageBitmap(blob);
      return { bitmap: bmp, width: bmp.width, height: bmp.height, source: 'photo' };
    } catch (_) { /* fall through */ }
  }
  if (imageCapture && imageCapture.grabFrame) {
    try {
      const bmp = await imageCapture.grabFrame();
      return { bitmap: bmp, width: bmp.width, height: bmp.height, source: 'grabFrame' };
    } catch (_) { /* fall through */ }
  }
  // Last resort: paint the <video> into a canvas
  if (!video.videoWidth) throw new Error('Camera not running yet');
  const c = document.createElement('canvas');
  c.width = video.videoWidth; c.height = video.videoHeight;
  c.getContext('2d').drawImage(video, 0, 0);
  return { bitmap: c, width: c.width, height: c.height, source: 'video' };
}

/**
 * Crop the title band from a captured frame, scale it up for OCR, and run
 * Otsu's algorithm for adaptive binarization. Otsu picks a per-image
 * threshold that separates background from text — much more robust than
 * the flat "stretch around 128" we used before.
 */
function cropTitleRegion(frame) {
  const { bitmap, width: vw, height: vh } = frame;
  if (!vw || !vh) return null;

  // The on-screen overlay marks: 8% inset on all sides, then top 13% of that inner box.
  const inset = 0.08;
  const bandH = 0.13;
  ...
  // Scale to a target height — OCR engines like ~50-80px tall text.
  // Standard MTG title cap-height is ~32-38px of an 80px band on a typical card capture.
  const targetH = 240;
  ...
  // Pass 1: grayscale → histogram for Otsu's threshold
  ...
  // Pass 2: binarize. MTG titles are dark text on a light cream/colored
  // background, so pixels above threshold → white background, below → black text.
  // For sets where the title bar is dark (some modern frames), we also produce
  // an inverted version below.
  for (let i = 0; i < d.length; i += 4) {
  ...
  // Also produce the inverted version for retry-on-fail
  const inv = document.createElement('canvas');
  inv.width = c.width; inv.height = c.height;
  const invCtx = inv.getContext('2d');
  invCtx.drawImage(c, 0, 0);
  const invImg = invCtx.getImageData(0, 0, inv.width, inv.height);
  const id = invImg.data;
  for (let i = 0; i < id.length; i += 4) {
    id[i] = 255 - id[i]; id[i+1] = 255 - id[i+1]; id[i+2] = 255 - id[i+2];
  }
  invCtx.putImageData(invImg, 0, 0);

  return { primary: c, inverted: inv, threshold };
}
...
/** Read text from a canvas using whichever engine is fastest/available. */
async function readText(canvas) {
  // Native first — runs in ~50-150ms on Android Chrome.
  if (NATIVE_TEXT_DETECTOR) {
    try {
      if (!nativeDetector) nativeDetector = new window.TextDetector();
      const blocks = await nativeDetector.detect(canvas);
      if (blocks && blocks.length) {
        // TextDetector returns blocks sorted by reading order on Android.
        // Pick the widest one as the title (titles span most of the band).
        blocks.sort((a, b) => b.boundingBox.width - a.boundingBox.width);
        return (blocks[0].rawValue || '').trim();
      }
    } catch (e) {
      console.warn('Native TextDetector failed, falling back:', e);
    }
  }
  // Tesseract fallback (or sole engine on iOS / older Android)
...
async function captureAndIdentify() {
  ...
  try {
    ...
    const frame = await grabBestFrame(video);

    const cropped = cropTitleRegion(frame);
    if (!cropped) throw new Error('Could not capture frame');

    setStatus(`Reading title (${frame.source}, ${frame.width}×${frame.height}, Otsu t=${cropped.threshold})…`);

    // Try the primary binarization first; if Scryfall can't find anything,
    // retry with the inverted version (in case of dark title bars).
    const guesses = [];
    let card = null;
    let lastErr = null;
    for (const canvas of [cropped.primary, cropped.inverted]) {
      const raw = await readText(canvas);
      const guess = cleanTitleText(raw);
      if (guess && guess.length >= 3) {
        guesses.push(guess);
        try {
          card = await scryfallLookup(guess);
          break;
        } catch (e) { lastErr = e; }
      }
    }
    ...
    if (!card) {
      const hint = guesses.length
        ? `OCR read “${guesses.join('” / “')}” — no match on Scryfall. Try better lighting or type the name below.`
        : `OCR couldn't read a name. Try better lighting, less glare, or type the name below.`;
      throw new Error(hint);
    }

    pushScan(card); // fire-and-forget
    showPhoneResult(card);
    const elapsed = Math.round(performance.now() - t0);
    setStatus(`Matched ${card.name} (${card.set_name}) in ${elapsed} ms.`, 'ok');
  } catch (e) {
...
async function doManualLookup() {
  ...
  try {
    ...
    pushScan(card); // fire-and-forget — don't make the user wait on the network sync
    showPhoneResult(card);
    const elapsed = Math.round(performance.now() - t0);
    setStatus(`Matched ${card.name} (${card.set_name}) in ${elapsed} ms.`, 'ok');
    input.value = '';
...
let glassesFbRef = null;
let glassesStorageListener = null;

function detachGlassesListener() {
  if (glassesFbRef) {
    try { glassesFbRef.off('value'); } catch (e) {}
    glassesFbRef = null;
  }
  if (glassesPollTimer) {
    clearInterval(glassesPollTimer);
    glassesPollTimer = null;
  }
  if (glassesStorageListener) {
    window.removeEventListener('storage', glassesStorageListener);
    glassesStorageListener = null;
  }
}

async function initGlasses(forceRebind) {
  if (initGlasses._bound && !forceRebind) {
  ...
  if (fbDb && getRoom()) {
    // Real-time: Firebase pushes updates as soon as the phone writes
    glassesFbRef = roomScansRef().limitToLast(MAX_RECENT);
    glassesFbRef.on('value', (snap) => {
      glassesState.scans = snapToArray(snap);
      if (glassesState.featuredIdx >= glassesState.scans.length) glassesState.featuredIdx = 0;
      updateHudStatus();
      renderFeatured();
      renderList();
    }, (err) => {
      console.warn('Firebase listener error:', err);
      // Fall back to polling so we still show something
      startGlassesPollFallback();
    });
  } else {
    // No Firebase → cross-tab 'storage' events + a slow poll
    glassesStorageListener = (e) => { if (e.key === STORAGE_KEY) refreshGlasses(); };
    window.addEventListener('storage', glassesStorageListener);
    startGlassesPollFallback();
  }
...
function startGlassesPollFallback() {
...
function updateHudStatus() {
  ...
  const n = glassesState.scans.length;
  if (n) {
    dot.classList.remove('idle');
    stat.textContent = `LIVE · ${n} CARD${n === 1 ? '' : 'S'}`;
  } else {
    dot.classList.add('idle');
    stat.textContent = fbDb && getRoom() ? 'WAITING · SUBSCRIBED' : 'WAITING';
  }
}
...
// Cleanup the OCR worker and Firebase listener on page unload
